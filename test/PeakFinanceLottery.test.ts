...
const PeakFinanceLottery = artifacts.require("./PeakFinanceLottery.sol");
...
contract("Lottery V2", ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
  ...
  let _priceTicketInPeak = parseEther("0.5");
  ...
  // Contracts
  let lottery, mockPeak, randomNumberGenerator;
  ...
  before(async () => {
    // Deploy MockPeak
    mockPeak = await MockERC20.new("Mock PEAK", "PEAK", _totalInitSupply);

    // Deploy MockRandomNumberGenerator
    randomNumberGenerator = await MockRandomNumberGenerator.new({ from: alice });

    // Deploy PeakFinanceLottery
    lottery = await PeakFinanceLottery.new(mockPeak.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve PEAK to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockPeak.mintTokens(parseEther("100000"), { from: thisUser });
        await mockPeak.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      expectEvent(result, "LotteryOpen", {
        ...
        priceTicketInPeak: _priceTicketInPeak.toString(),
        firstTicketId: "0",
        injectedAmount: "0",
      });
    ...
    it("Bob buys 100 tickets", async () => {
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, mockPeak, "Transfer", {
        from: bob,
        to: lottery.address,
    ...
    it("Owner does 10k PEAK injection", async () => {
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 PEAK collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 PEAK
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 PEAK collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 PEAK
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 PEAK
    ...
    describe("LOTTERY #2 - CUSTOM RANDOMNESS - Exceptions", async () => {
      ...
      it("Operator cannot start lottery if length is too short/long", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTimeTarget, _priceTicketInPeak, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if discount divisor is too low", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTime, _priceTicketInPeak, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if ticket price too low or too high", async () => {
        let newPriceTicketInPeak = parseEther("0.0049999999");

        await expectRevert(
          lottery.startLottery(endTime, newPriceTicketInPeak, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
          "Outside of limits"
        );

        newPriceTicketInPeak = parseEther("0.0049999999");
      ...
      it("Operator cannot start a second lottery", async () => {
        await expectRevert(
          lottery.startLottery(_lengthLottery, _priceTicketInPeak, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not PEAK token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockPeak.address, parseEther("1"), { from: alice }),
          "Cannot be PEAK token"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockPeak.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
