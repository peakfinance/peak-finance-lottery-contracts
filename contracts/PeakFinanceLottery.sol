...
contract PeakFinanceLottery is ReentrancyGuardUpgradeable, IPeakFinanceLottery, OwnableUpgradeable {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPeak;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] peakPerBracket;
        ...
        nonReentrant
    {
        ...
        // Calculate number of PEAK to this contract
        uint256 amountPeakToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPeak,
            _ticketNumbers.length
        );

        // Transfer peak tokens to this contract
        peakToken.safeTransferFrom(address(msg.sender), address(this), amountPeakToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPeak += amountPeakToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPeakToTransfer
        uint256 rewardInPeakToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPeakToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        peakToken.safeTransfer(msg.sender, rewardInPeakToTransfer);

        emit TicketsClaim(msg.sender, rewardInPeakToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 randomNumber = _random() % 1000000;
        uint32 finalNumber = uint32(randomNumber);

        // Initialize a number to count addresses in the previous bracket
        uint256 numberAddressesInPreviousBracket;

        // Calculate the amount to share post-treasury fee & 10% Burn
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPeak) * (10000 - _lotteries[_lotteryId].treasuryFee - 1000))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury = _lotteries[_lotteryId].amountCollectedInPeak * _lotteries[_lotteryId].treasuryFee / 10000 ;
        uint256 amountToBurn = _lotteries[_lotteryId].amountCollectedInPeak * 10 / 10000 ;

        // Calculate prizes in PEAK for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].peakPerBracket[j] =
                ...
                // A. No PEAK to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].peakPerBracket[j] = 0;
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in PEAK token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        peakToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPeak += _amount;
     ...
     * @param _priceTicketInPeak: price of a ticket in PEAK
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPeak >= minPriceTicketInPeak) && (_priceTicketInPeak <= maxPriceTicketInPeak),
            "Outside of limits"
        );
        ...
        require(
            (_rewardsBreakdown[0] +
                ...
                _rewardsBreakdown[3] +
                _rewardsBreakdown[4] +
                _rewardsBreakdown[5]) == 8000,
            "Rewards must equal 8000"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPeak: _priceTicketInPeak,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            peakPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
    ...
    /**
     * @notice Set PEAK price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPeak: minimum price of a ticket in PEAK
     * @param _maxPriceTicketInPeak: maximum price of a ticket in PEAK
     */
    function setMinAndMaxTicketPriceInPeak(uint256 _minPriceTicketInPeak, uint256 _maxPriceTicketInPeak)
        external
        onlyOwner
    {
        require(_minPriceTicketInPeak <= _maxPriceTicketInPeak, "minPrice must be < maxPrice");

        minPriceTicketInPeak = _minPriceTicketInPeak;
        maxPriceTicketInPeak = _maxPriceTicketInPeak;
    }
     ...
     * @param _priceTicket price of a ticket (in PEAK)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].peakPerBracket[_bracket];
        } else {
            return 0;
        }
    }
